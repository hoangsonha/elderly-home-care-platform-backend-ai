...
public class PaymentController {
     ...
     * gọi liên tục api "GET: /api/v1/payments/order/{orderId}" sau mỗi 1 hoặc 2s để kiểm tra xem người dùng đã thanh toán hay chưa,
     * có 3 status là PAID (thanh toán thành công), CANCELLED (hủy), PENDING (tiếp tục kiểm tra), nếu quá 30p mà chưa thanh toán
     * thì coi như hủy và gọi api "PUT: /api/v1/payments/order/{orderId}" để hủy đơn hàng
    */
    ...
    public ResponseEntity<ObjectResponse> createPaymentLinkWithQR(
        ...
        try {
          ...
          return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Create link payment with QR Code successfully", response)) :
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Create link payment with QR Code failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> getOrderById(@PathVariable("orderId") long orderId, CreatePaymentSuccess requestBody) {
        try {
            ...
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Check status order failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> cancelOrder(@PathVariable("orderId") long orderId) {
        try {
            ...
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Cancel order successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Cancel order failed", null));
        } catch (Exception e) {
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Cancel order failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> payosTransferHandler(@RequestBody Object body) {
        try {
          WebhookData response = payOSService.payosTransferHandler(body);
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "PayOS transfer handler successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "PayOS transfer handler failed", null));
        } catch (Exception e) {
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "PayOS transfer handler failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> creatOrderLink(
        ...
        try {
            ...
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Create order link successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Create order link failed", null));
        } catch (Exception e) {
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Create order link failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> confirmWebhook(@RequestBody Map<String, String> requestBody) {
        try {
            ...
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Confirm webhook successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Confirm webhook failed", null));
        } catch (Exception e) {
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Confirm webhook failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> retrieveInvoices(@PathVariable("orderId") long orderId) {
        try {
            ...
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Get invoices successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Get invoices failed", null));
        } catch (Exception e) {
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Get invoices failed. " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<?> downloadInvoice(
        ...
        try {
          ResponseEntity<?> response = payOSService.downloadInvoice(orderId, invoiceId);
            return response != null ? ResponseEntity.status(HttpStatus.OK).body(new ObjectResponse("Success", "Download invoices successfully", response)) :
                  ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Download invoices failed", null));
            ...
            log.error("Error found", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", "Download invoices failed. " + e.getMessage(), null));
        }
    }
